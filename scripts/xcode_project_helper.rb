...
def get_targets_for_file(project, file_path)
  result = []
  project.native_targets.each do |target|
    # puts "Traversed Target: \"#{target}\""
    # puts "get_targets_for_fle sample_file_path: \"#{target.source_build_phase.files_references.first&.real_path&.to_s}\""
    target.source_build_phase.files_references.each do |file|
      # puts "file: \"#{file.real_path.to_s}\""
      result << target.name if file.real_path.to_s == file_path
    end
  end
  # show if "None" if the file doesn't belong to any target
  result = ["None"] if result.empty?
  result
end
...
# rubdocop:enable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity,
